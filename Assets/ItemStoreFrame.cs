...
public class ItemStoreFrame : MonoBehaviour
{
    ...
    public void OpenStore()
    {
        ...
        GameObject[] saleItems_Npc = Manager.instance.playerController.
            target.GetComponent<NPC_Store>().saleItems;
        ...
        for(int i =0;i<saleItems_Npc.Length; i++)
        {
            if(saleItems_Npc[i] !=null)
            {
                ...
                obj.transform.parent.GetChild(0).GetComponent<TextMeshProUGUI>().
                    text = string.Format("{0}G", obj.GetComponent<Items_Info>().price);
