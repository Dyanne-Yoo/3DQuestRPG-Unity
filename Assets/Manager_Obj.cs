...
public class Manager_Obj : MonoBehaviour
{
    ...
    GameObject AddObj(GameObject prefab, Transform pool,
    ...
    public GameObject GetObj(List<GameObject>list)
    {
        ...
        {
            GameObject obj = AddObj(prefab_Skill_SoulArrow,
                pool_SkillObj, list_Skill_Arrow);
            return obj;
        }
        if (list == list_hitEffect)
        {
            GameObject obj = AddObj(prefab_hitEffect,
                pool_Effect, list_hitEffect);
            return obj;
        }
        if(list == list_dmgText)
        {
            GameObject obj = AddObj(prefab_dmgText,
                pool_text, list_dmgText);
            return obj;
        }
        return null;
    }
