...
public class PlayerController : MonoBehaviour,IPointerDownHandler
{
    ...
    {
        Manager.instance.manager_SE.seAudio.PlayOneShot(
            Manager.instance.manager_SE.casting);
        ...
        castingBar.transform.GetChild(0).
            GetComponent<TextMeshProUGUI>().text = skillName;
        ...
        while (true)
        {
            ...
            if(time >= castingTime)
            {
                Manager.instance.manager_SE.seAudio.Stop();
    ...
    void Targeting()
    {
        ...
        hpBar_Target.SetActive(target.GetComponent<Obj_Info>().
            type == "Enemy");
    ...
    void OnTarget()
    {
        if(target!=null)
        {
            ...
            if (target_Tool.activeSelf)
                target_Tool.transform.position =
                    cam.WorldToScreenPoint(target.position + new Vector3(0, 2, 0));
        }
    }
    ...
    private void Update()
    {
        ...
        if(target!= null&&target.gameObject.tag == "Enemy")
        {
            ...
            hpBar_Target.transform.GetChild(0).GetComponent<Image>().
                fillAmount = enemyState.curHp / enemyState.hp;
        }

        if(target != null && target.gameObject.tag == "NPC" &&
            targetDis < 2.5f && clickNpc)
        {
            clickNpc = false;
            target.GetComponent<NPC_Dialog>().Dialog();
        }

        if (target != null && target.gameObject.tag == "Merchant" &&
            targetDis < 2.5f && clickNpc)
        {
            clickNpc = false;
            Manager.instance.manager_Inven.storeFrame.GetComponent<
                ItemStoreFrame>().OpenStore();
